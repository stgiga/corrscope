...
class CorrelationTrigger(MainTrigger):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._corr_kernel = np.zeros(self.A + self.B, dtype=f32)
    ...
    def get_trigger(self, pos: int, cache: "PerFrameCache") -> TriggerResult:
        ...
        # array[A+B] Amplitude
        corr_kernel: np.ndarray = self._corr_kernel * self.cfg.buffer_strength
    ...
    def spectrum_rescale_buffer(self, data: np.ndarray) -> None:
        ...
        # Setup
        scfg = self.scfg
        Ntrigger = self._corr_kernel.size
        ...
        prev_spectrum = calc_spectrum(self._corr_kernel)
    ...
    def _update_buffer(self, data: np.ndarray, cache: PerFrameCache) -> None:
        ...
        N = len(data)
        if N != self._corr_kernel.size:
            raise ValueError(
                f"invalid data length {len(data)} does not match "
                f"CorrelationTrigger {self._corr_kernel.size}"
            )
